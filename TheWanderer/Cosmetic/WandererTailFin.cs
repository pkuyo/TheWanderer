    ...
    public class WandererTailFin : CosmeticBase
    {
        ...
        public override void DrawSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                {
                    float num2 = Mathf.InverseLerp(startSprite, startSprite + bumps - 1, j);
                    ...
                    sLeaser.sprites[j + num].rotation = Custom.VecToDeg(Vector2.Lerp(spineData.perp * spineData.depthRotation, spineData.dir * ((i != 1) ? 1 : -1), num2));
                    ...
                    if (colored)
                    {
                        ...
                        sLeaser.sprites[j + bumps + num].rotation = Custom.VecToDeg(Vector2.Lerp(spineData.perp * spineData.depthRotation, spineData.dir * ((i != 1) ? 1 : -1), num2));
        ...
        public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                for (int j = startSprite + bumps - 1; j >= startSprite; j--)
                {
                    Mathf.InverseLerp(startSprite, startSprite + bumps - 1, j);
        ...
        public int bumps;
        public int graphic;
        readonly float scale = 0.66f;
        readonly float sizeMin = 0.3f;
        readonly float thickness = 0.7f * 3f;
        readonly float sizeExponent = 0.6f;
        readonly float spineLength = 0.6f;
        readonly float undersideSize = 0.6f;
        readonly float ScaleY = 3f;
        readonly float ScaleX = 1f;
