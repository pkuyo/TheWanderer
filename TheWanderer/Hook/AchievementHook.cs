...
namespace Pkuyo.Wanderer
{
    class AchievementHook : HookBase
    {
        ...
        {
            orig(self,endGameID);
            if (endGameID == WandererEnum.WinState.Dragonlord)
        ...
        {
            var re = orig(ID);
            if (ID == WandererEnum.WinState.Dragonlord)
        ...
        private void WinState_CycleCompleted(On.WinState.orig_CycleCompleted orig, WinState self, RainWorldGame game)
        {
            ...
            if (game.session.creatureCommunities.playerOpinions[CreatureCommunities.CommunityID.Lizards.Index - 1, 0, 0] > 0 || game.session.characterStats.name.value == WandererMod.WandererName)
                floatTracker = self.GetTracker(WandererEnum.WinState.Dragonlord, true) as WinState.FloatTracker;
            else
                floatTracker = self.GetTracker(WandererEnum.WinState.Dragonlord, false) as WinState.FloatTracker;
